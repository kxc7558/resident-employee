...
`--trial-engine` 是**试用引擎**：不装模型、不花钱、不起进程，就能把页面走一遍，
...
| 要什么 | 实现了什么 |
|---|---|
| **对话界面** | 多会话（列表／新建／切换／删除）、流式逐字、Markdown/表格/代码渲染、深色模式 |
| **过程可见** | 调了什么、被拦了什么，**当场**显示在回复上方（可折叠的过程清单） |
| **配置页** | 模型后端、令牌、工具面、步数上限都在界面上改——不改文件 |

外加**人在环**，而且把两类"被拦"分清——这一条很重要：

| 被拦原因 | 能不能批准 | 前台怎么表现 |
|---|---|---|
| **缺授权**（授权闸门拦的） | **能** → 点「确认执行」签一张一次性授权，只覆盖那一个动作 | 「⚠️ 需要你确认」+ 按钮 |
| **不在工具面 / 触发循环检测** | **不能**，批准也没用——重跑还是会被同一道闸拦住 | 「⛔ 这几个批准也没用」+ 说明要改配置 |

**给用户一个点了没用的按钮，比不给更糟**，所以这两类必须分开报。

**两条安全约定**：

- **配置里的令牌从不回传**——`GET /api/config` 只说"有没有配"，不说"是什么"
- **批准只认会话记录里真的被拦过的动作**——不能"客户端说批什么就批什么"，
  否则任何本机页面都能给自己签一个从没被拦过的动作。这条同时让「确认执行」按钮
  **刷新页面后还在**（可确认的动作从记录里推，不是内存里的临时状态）
...
| **运行时**（本仓库） | 把员工跑起来（闸门 + 事件流 + 审计） | ✅ v0.1 |
| **网页前台**（本仓库） | 把员工摆到人面前（对话 + 过程可见 + 人在环） | ✅ v0.1，181 用例 / 覆盖 90%+ |
